#!/usr/bin/env python3
# v9.0 正式版全量回归测试 — 覆盖 v7.3~v8.6 所有关键功能
import sys
...
with sync_playwright() as p:
    ...
    check("versionTag == v9.0", page.text_content("#versionTag").strip() == "v9.0")

    page.click("#modeStoryBtn")
    page.wait_for_timeout(1200)

    # pigTrigger 必须最先（会创建猪的测试会污染）
    ...
    check("coconut no-boulder", page.evaluate("window.__fpsTest.coconutTest()")["ok"])

...
(function (global) {
  ...
  global.MODELS.walkie = {
    ...
    create: function (config) {
      ...
      const bodyMat = new window.MARIO.matS({ color: 0x2f3b2f});
      const darkMat = new window.MARIO.matS({ color: 0x1a1f1a});
      const accentMat = new window.MARIO.matS({ color: 0xd9412e, emissive: 0xd9412e, emissiveIntensity: 0.35 });
      const screenMat = new window.MARIO.matS({ color: 0x9fe8c8, emissive: 0x52d9a2, emissiveIntensity: 0.9});
      ...
      const ring = new T.Mesh(
        ...
        new window.MARIO.basicS({ color: 0x66ccff, transparent: true, opacity: 0.85 })
      );
      ring.rotation.x = Math.PI / 2;
      g.add(ring);
      const glow = new T.Mesh(
        new T.SphereGeometry(0.42, 10, 8),
        new window.MARIO.basicS({ color: 0x66ccff, transparent: true, opacity: 0.16 })
      );
      g.add(glow);

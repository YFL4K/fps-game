...
(function (global) {
  ...
  global.MODELS.pickup = {
    ...
    create: function (config) {
      ...
      const baseMat = new window.PIXEL.matCompat({
      ...
      // 十字标记
      const markMat = new window.PIXEL.matCompat({ color: 0xffffff, emissive: 0xffffff, emissiveIntensity: 0.8 });
      ...
      const ring = new T.Mesh(
        new T.BoxGeometry((2*(0.28+0.025)), (2*0.025), (2*(0.28+0.025))),
        new window.PIXEL.basicCompat({ color: isFlame ? 0xff8844 : (kind === 'health' ? 0x7dffc0 : 0xffe08a) })
      );

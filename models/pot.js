...
(function (global) {
  ...
  global.MODELS.pot = {
    ...
    create: function (config) {
      ...
      const g = new T.Group();
      const potMat = new window.MARIO.matS({ color: potColor});
      const soilMat = new window.MARIO.matS({ color: 0x3d2817});
      const plantMat = new window.MARIO.matS({ color: 0x228b22});
      ...
      if (isLarge) {
        ...
        const flower = new T.Mesh(
          ...
          new window.MARIO.matS({ color: 0xff6b9d})
        );

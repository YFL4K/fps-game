...
(function (global) {
  ...
  global.MODELS.shield = {
    ...
    create: function (config) {
      ...
      const gold = new window.MARIO.matS({
        color: 0xf5c518,
        emissive: 0x8a6d00, emissiveIntensity: 0.55
      });
      const goldDark = new window.MARIO.matS({
        color: 0xb8860b,
      ...
      const glow = new T.Mesh(new T.RingGeometry(0.22, 0.32, 24),
        new window.MARIO.basicS({ color: 0xffe066, transparent: true, opacity: 0.35, side: T.DoubleSide, depthWrite: false }));

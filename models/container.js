...
(function (global) {
  ...
  global.MODELS.container = {
    ...
    create: function (config) {
      ...
      const g = new T.Group();
      const containerMat = new window.MARIO.matS({ color: color});
      const corrugMat = new window.MARIO.matS({ color: new T.Color(color).multiplyScalar(0.85).getHex()});
      const doorMat = new window.MARIO.matS({ color: new T.Color(color).lerp(new T.Color(0xffffff), 0.15).getHex()});
      const cornerMat = new window.MARIO.matS({ color: 0x222222});

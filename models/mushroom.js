  ...
  global.MODELS.mushroom = {
    ...
    create: function (config) {
      ...
      const stem = new T.Mesh(
        ...
        new window.MARIO.matS({ color: stemColor, flatShading: true })
      );
      ...
      const cap = new T.Mesh(
        ...
        new window.MARIO.matS({ color: capColor, flatShading: true })
      );
      cap.position.y = 0.55;
      cap.scale.y = 0.72;
      ...
      // 菌盖白斑（毒蝇伞/红斑菇）
      if (spotted) {
        const spotMat = new window.MARIO.matS({ color: 0xffffff});
        for (let i = 0; i < 5; i++) {

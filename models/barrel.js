...
(function (global) {
  ...
  global.MODELS.barrel = {
    ...
    create: function (config) {
      ...
      const bodyMat = new window.MARIO.matS({ color: cfg.color || 0xc0392b});
      const ringMat = new window.MARIO.matS({ color: 0x2c3e50});
      const stripeMat = new window.MARIO.matS({
        color: 0xf1c40f,
      ...
      // 黑色危险标签
      const warn = new T.Mesh(new T.BoxGeometry(0.46, 0.2, 0.02), new window.MARIO.basicS({ color: 0x141414 }));
      warn.position.set(0, 0.68, 0.522);
      const warn2 = new T.Mesh(new T.BoxGeometry(0.02, 0.2, 0.46), new window.MARIO.basicS({ color: 0x141414 }));

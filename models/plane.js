...
(function (global) {
  ...
  global.MODELS.plane = {
    ...
    create: function (config) {
      ...
      const g = new T.Group();
      const bodyMat = new window.MARIO.matS({ color: color});
      const darkMat = new window.MARIO.matS({ color: 0x222222});
      const glassMat = new window.MARIO.matS({ color: 0x3366aa, transparent: true, opacity: 0.65 });
      ...
      // 起落架支柱
      const strutMat = new window.MARIO.matS({ color: 0x444444});
      ...
      const stripe = new T.Mesh(new T.CylinderGeometry(0.77, 0.67, 7.8, 12, 1, true), 
        new window.MARIO.matS({ color: stripeColor, side: T.DoubleSide }));

...
(function (global) {
  ...
  global.MODELS.weapon = {
    ...
    create: function (config) {
      ...
      const bodyMat = new window.MARIO.matS({ color: 0x22252b});
      const accentMat = new window.MARIO.matS({
        color: look.color,
        emissive: look.color, emissiveIntensity: 0.5
      });
      const woodMat = new window.MARIO.matS({ color: 0x6b4423});
      ...
      const ring = new T.Mesh(
        ...
        new window.MARIO.basicS({ color: look.color, transparent: true, opacity: 0.7 })
      );

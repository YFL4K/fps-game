...
(function (global) {
  ...
  global.MODELS.grass = {
    ...
    create: function (config) {
      ...
      const baseMat = new window.MARIO.mat({ color: 0x4c8c3f});
      const tuftMat = new window.MARIO.mat({ color: 0x6aa84f});
      ...
      for (let i = 0; i < 6; i++) {
        ...
        g.add(stem);
        const head = new T.Mesh(new T.SphereGeometry(0.09 * s, 6, 5), new window.MARIO.mat({ color: flowerColors[i % flowerColors.length]}));

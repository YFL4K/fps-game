...
一个**零依赖、纯前端、程序化生成**的第一人称射击游戏（当前版本 **v11.27**）。双击 `index.html` 即可游玩 —— 无服务器、无构建、无安装。过关模式（5 关）+ 无尽模式，墙体掩体结构、全场景可破坏、猪头佳/机甲 BOSS 双 BOSS 战。
...
- 🐗 **5 规格参数按新表重定**（`models/pig.js` 的 `PIG_VARIANTS`）：
  ...
  |---|---|---|---|---|---|---|
  | 1 | 瘟疫猪头佳 | 0.5×（1.5） | ×0.7 | 灰棕 | 白 | 35% |
  | 2 | 撼地尊猪头佳 | 1×（3） | ×1 | 原色深褐 | 绿 | 30% |
  | 3 | 镇海兽猪头佳 | 2×（6） | ×2.5 | 暗灰 | 蓝 | 10% |
  | 4 | 灾厄猪头佳 | 3×（9） | ×3.5 | 暗青 | 紫 | 10% |
  | 5 | 灭世猪头佳 | 4×（12） | ×5 | 黑 | 红 | 5% |
...
- ✅ 语法括号配平 5689/5689；headless 实测 `pigVariantTest` 变体 scale=1.5/3/6/9/12、hp=38500/55000/137500/192500/275000 全命中；端到端 `spawnPigVariantTest` 走完整 `spawnPig→buildEntity→applyTransform` 链路 scale=1.5/3/6/9/12 全命中。

...
一个**零依赖、纯前端、程序化生成**的第一人称射击游戏（当前版本 **v11.48**）。双击 `index.html` 即可游玩 —— 无服务器、无构建、无安装。过关模式（5 关）+ 无尽模式，墙体掩体结构、全场景可破坏、猪头佳/机甲 BOSS 双 BOSS 战。
...
### v11.48（2026-09-20）— 💥 爆炸物伤害大幅提升

- 🔥 **TNT**：爆炸范围 6.5→**26**（×4），伤害 100→**400**（×4）
- 🛢 **油桶**：爆炸范围 5→**10**（×2），伤害 70→**210**（×3）
- ⚡ **连环引爆**：范围 5.5→**11**，伤害 60→**180**（同步提升）
...
- 🐛 **加特林报错修复**：`st is not defined` → 改用 `gatlingRec.cfg.id`
- 🏗 **瞭望塔修复**：平台碰撞体 3.2×2.0→3.2×3.2（匹配视觉平台），加特林位置修正，瞬移位置优化
- 🎯 **爆头秒杀10%**：所有敌人（含BOSS和猪头佳）命中头部10%概率一击必杀
- 💥 **武器伤害调整**：手枪78→140、步枪46→180、狙击枪390→500、狙击爆炸195→800
- 📏 **机甲BOSS激光射程**：200m→50m（所有等级统一）

...
一个**零依赖、纯前端、程序化生成**的第一人称射击游戏（当前版本 **v10.4**）。双击 `index.html` 即可游玩 —— 无服务器、无构建、无安装。过关模式（5 关）+ 无尽模式，墙体掩体结构、全场景可破坏、猪头佳/机甲 BOSS/霸王龙三大 BOSS 战。
...
- 🔫 **5 种武器**（枪模程序化还原真实枪型，数值均为当前 v10.4 版本）：
  - **手枪**（沙漠之鹰）：伤害 60 / 6 发每秒 / 12 发弹匣
  - **步枪**（AK-47）：伤害 35 / 15 发每秒 / 30 发弹匣，开局自带
  - **喷火器**：持续伤害 **300/秒**，射程 **80**，火焰范围半径 **12**
  - **狙击枪**（AWP）：伤害 300 / 3 发每秒，右键 **4 倍瞄准镜**
  - **火箭筒**：伤害 210 / 爆炸 255，AoE 范围伤害，射程 200
  - 数字键 `1-5` 或**鼠标滚轮**切换；所有武器右键瞄准；击毁直升机掉落火箭筒
- 🧱 **墙体掩体结构**：每张随机地图自动生成 **走廊 / 胡同 / L 形拐角 / 独立直墙段**（26~42 面掩体墙），供枪战走位
- 💥 **全物品可破坏**：墙体、房子、箱子（**1500 血量**，需伤害累加才会彻底破坏）及车辆、集装箱、货柜、大树等场景物品均可被武器打碎（含碎片特效）；火箭筒/油桶/TNT 爆炸范围破坏
- 🐗 **猪头佳小BOSS**：每击杀 10 名普通敌人随机出现，**55000 血量**、移动速度 2.7，双眼**绿色激光** + 无差别冲撞，**优先攻击玩家**；被障碍卡住时会**撞碎障碍物**继续追杀
- 🤖 **机甲 BOSS**：每击杀 25 名普通敌人随机出现（所有关卡+无尽），火箭炮 AoE、防御×2、移动速度 2.7，被障碍卡住会撞开障碍物；击败奖励核弹 + 空袭对讲机
- 🦖 **霸王龙**：20000 分随机出现，红眼黑皮、体型巨大、60 秒超时自爆
- 👤 **三类普通敌人**：人类士兵 / 重装怪 / 蜘蛛（自爆）+ **直升机**（火箭弹 AoE，击毁掉武器/核弹/对讲机）
- 📻 **空中支援**：击杀 BOSS/直升机掉落对讲机，按 `T` 呼叫导弹空袭（可累积 **3 次**）；每获得 5000 分额外奖励空袭 ×1 + 满血恢复
- ☢️ **核弹**：宝箱/猪头佳/BOSS/直升机均可掉落，按 `G` 引爆毁灭全场敌人与场景，对玩家自伤 30%
...
- 🌳 **程序化场景**：建筑、车辆、集装箱、飞机、大树、高墙、摩天轮、蘑菇、草地花丛、2 种飞鸟随机分布
- 🔊 **程序化战斗 BGM**：每关一首 WebAudio 实时合成劲爆战斗曲（L1-L5 递增），无尽模式专属曲，`M` 键静音
- ⚡ **性能优化**：全场景 MeshLambertMaterial、阴影关闭、特效对象池（tracer/spark/flame 复用）、pixelRatio 限制、烟雾精简，运行流畅稳定
...
### v10.6（2026-09-11）— 🎨 超级马里奥3D世界美术风格重构
- 🎨 **全场马里奥3D世界卡通美术风格**：圆润饱满曲面 + 高饱和任天堂调色板 + 塑胶玩具质感（MeshStandardMaterial 高光）
- 🐗 **猪头佳**：圆润猪身 + 凶狠黄瞳/锐利眉骨 + 象牙白獠牙 + 皮质战术背心（金属扣件）
- 🤖 **机甲 BOSS**：酷霸王机甲风（刺球肩膀 + 履带重足 + 半透明暗红座舱盖 + 发光核心）
- 🦖 **恐龙**：马里奥奥德赛恐龙（饱满肉感 + 白亮牙齿 + 黄色瞳孔 + 眉骨 + 圆角背刺）
- 🚗 **车辆卡丁车**：圆润胶囊车身 + 玻璃圆顶 + 亮金轮毂
- ☀️ **天空**：湛蓝渐变 + 棉花糖白云 + 圆太阳
- 🏠 **建筑**：圆形发光窗 + 拱形门

### v10.4（2026-09-10）— 🔥 喷火器强化 + 键位调整
- 🐗 **猪头佳/机甲 BOSS 移动速度 +170%**：由 speed 1 提升至 **2.7**（更快接近玩家）
- 🔥 **喷火器大幅强化**：伤害 100/秒 → **300/秒**，射程 60 → **80**，火焰范围半径 9 → **12**
- 🛡️ **金色护盾掉落提升**：击杀普通敌人掉落概率 10% → **20%**
- ✈️ **喷气背包启动键调整**：由空格键改为 **B 键**（空格保留为跳跃）

### v10.3（2026-09-10）— 🔧 破坏血量与大型敌人移动修复
- 💥 **墙/房/箱生命值提升至 1500**：墙体、房子、箱子不再被一枪打碎，需要伤害累加（多次命中）才会彻底破坏，其余物品保持各自血量
- 🐗 **大型敌人撞击破坏修复**：猪头佳、机甲 BOSS 前进时会主动检测并撞开前方障碍物（墙/房/箱），不再卡在角落无法移动，能随机跑动追杀玩家
- 🐌 **移动速度下调**：猪头佳、机甲 BOSS 移动速度降低到与普通敌人一致（speed 1），避免移动过快

### v10.2（2026-09-10）— 💥 全物品可破坏 + 大型敌人撞击破坏
- 🌴 **删除椰子树**：彻底移除 coconut.js 模型、场景生成、script 加载、测试钩子等所有相关代码
- 💥 **全物品可破坏**：地图所有静态物品（房子、箱子、墙体、高墙、集装箱、货车、飞机、大树、灯柱、花盆、宝箱、摩天轮等）均可被玩家武器攻击破坏；按物品类型设定血量（建筑 300 / 墙 120 / 箱子 60 等），命中扣血、血尽破碎（含碎片特效）
- 🔫 **玩家武器破坏**：所有枪械命中静态物品会扣血，多次命中可打碎建筑/墙体；火箭筒、油桶、TNT 爆炸范围破坏更多物品（扩展 DESTRUCTIBLE_MODELS）
- 🐗 **猪头佳优先攻击玩家**：取消 v8.3「优先攻击机甲 BOSS」设定，改为玩家存活时始终锁定玩家，玩家死亡后才攻击其他目标
- 🦖 **大型敌人撞击破坏**：猪头佳、机甲 BOSS 被场景物品卡住无法移动时，持续撞击（每 0.45 秒 90 伤害）会撞碎障碍物（墙/建筑/箱子/集装箱等），从而随机跑动追杀玩家；地图四周边界墙标记为不可破坏，防止敌人越界
...
### v9.2（2026-09-08）— 🌸 樱花树优化 + 枪械纯净视角
- 🌸 **樱花树重做**：修复树干错位（直栈式分段，底部对齐上段顶部）；3 层花冠（底/中/顶由宽到窄），完全覆盖所有树枝；花簇球面均匀分布，树型优美；花瓣粒子 24 片循环飘落
- 🚫 **取消手部模型**：删除持枪双手（buildHands），屏幕上不再显示玩家手部模型和动作——纯净枪械视角
- 🚫 **取消准星发光点**：删除枪管上的红/绿色准星发光标识（sightGlow），枪械外观更干净
- ✅ **枪模完整显示**：确保所有枪械在左键开火和右键瞄准状态下都完整显示无残缺（frustumCulled=false + depthTest=false + renderOrder=999 三件套保障）
- 🎨 **缓存破坏**：sakura.js/gun.js/scene-layout.js 加版本号参数防浏览器缓存

### v9.1（2026-09-08）— 🌸 樱花树 + CS 风格枪械重做
- 🌸 **樱花树替代椰子树**：删除椰子树（coconut.js），新增精美樱花树（sakura.js）——深褐弯曲树干 + 伸展枝桠 + 多层次粉樱花花簇（8 种粉色色阶）+ 飘落花瓣粒子动画（28 片花瓣循环飘落 + 风中摇摆）；大/中/小三种尺寸随机分布
- 🔫 **CS 风格枪械全面重做**：六把枪全部参考 CS/CS:GO 真实枪械比例与外观重新建模：
  - **沙漠之鹰 .50AE**：棱角滑套 + 6 道纵向棱纹枪管 + 战术导轨 + 红点准星 + 制退器
  - **AK-47**：冲压机匣 + 顶盖加强筋 + 木质下护木 + 弧形弹匣 + 斜切消焰器 + 大拨片拉机柄
  - **Nova 霰弹枪**：木质泵动护木（带防滑纹）+ 双管轮廓 + 弹仓管 + 贴腮板枪托 + 皮卡汀尼导轨
  - **喷火器**：双燃料罐（主罐战术橙 + 副罐钢色）+ 黄铜阀门 + 气压表 + 引火管 + 橡胶握把
  - **AWP 狙击**：长枪管 + 消音器（环形槽纹理）+ 大倍率镜（遮光罩 + 透镜 + 调节旋钮）+ 两脚架 + 战术可调枪托
  - **火箭筒**：粗发射管 + 喇叭口 + 战术橙弹头 + 光学瞄具 + 前握把 + 橡胶肩垫
- 🎨 **材质体系重建**：新增枪械钢（高金属度暗灰）、聚合物（哑黑）、FDE 沙色、铬钢（亮金属枪管）、黄铜（弹匣/阀门）、战术橙、黑橡胶（握把防滑）等 11 种材质；每把枪拥有多色搭配与金属质感
- ✋ **双手持枪恢复**：恢复 CS 标准双手视角（右手握把 + 左手护木），手套材质（黑色战术手套）
...
### v8.6（2026-09-06）
- 🦖 **霸王龙外观重做**：重新建模为逼真霸王龙（巨大颅骨+前突吻部+两排利齿+张嘴下颚+红色发光眼、前倾粗壮身体、水平伸展渐细长尾、粗壮后腿带三趾脚掌、短小前肢+双爪）；体型缩小 50%（scale 3.2→1.6）；出现频率 1000 分 → **20000 分**；移动速度 +60%（5→8）
- 🔫 **持枪显示修复（参考 CS）**：恢复完整枪械显示——枪托不再空缺，整把枪（枪管+枪身+枪托）完整显示，右键瞄准也完整显示（不再只显示枪管）

### v8.5（2026-09-06）
- 📊 **游戏结算画面**：死亡/通关时显示详细数据——各类敌人击杀数（人类/怪物/蜘蛛/猪头佳/机甲BOSS/霸王龙/直升机）、爆头率、命中率、总击杀，以及**历史最高分排名（前 5，localStorage 持久化）**
- 🔦 **猪头佳激光 20 度角**：激光改为与地面呈固定 20 度角（不再 35°~75° 动态扫射）
- 🌴 **椰子树替代巨石**：删除巨石模型（boulder.js），新增**精美椰子树**（微弯棕榈树干 + 放射状下垂棕榈叶 + 挂果椰子），大/中/小三种尺寸随机分布
- 🔫 **自动拾取枪不切换**：拾取地图上的枪不再自动切换武器，只给对应枪增加备弹（新增分枪备弹池 ammoPool，切枪时保存/恢复）

### v8.4（2026-09-06）
- ✈️ **喷气飞行器道具**：击杀普通敌人 5% 掉落（蓝色飞机标志）+ 每 5000 分自动获得 1 个（不累积，上限 1，用完作废）；按空格启动向上飞行，飞行高度钳制到机甲 BOSS 头顶平齐，有效 15 秒，超时掉落
- 🎯 **狙击步枪强化**：射速 1 发/秒 → **3 发/秒**，换弹 2 秒 → **1 秒**
- 🤖 **机甲 BOSS 紫色激光**：所有机甲 BOSS 新增紫色激光武器（从头部向玩家穿透射线），伤害与猪头佳激光一致（玩家 30/s、路径敌人 50/s）
- 🦖 **霸王龙敌人**：红眼黑灰皮肤、体型与机甲 BOSS 相当；每 1000 分随机出现（全关卡+无尽）；向地面喷火 + 踩踏（对玩家/敌人无差别伤害 + 毁坏地图物品）；行走路径吃地图蘑菇；血量 24000（与猪头佳一致），60 秒超时自爆
- 🔫 **枪模改造**：删除持枪手型；画面只显示半个枪身（枪管+枪身），枪托不显示；右键瞄准（除狙击）只显示枪管+准星

### v8.3（2026-09-06）
- 🐗 **猪头佳防弹增强**：对玩家子弹伤害防护 ×2（defense 1 → 2，即减伤 50%）
- 🎯 **猪头佳优先攻击 BOSS**：地图中有 BOSS 机甲存活时，猪头佳冲撞/激光优先锁定 BOSS（不再优先玩家/普通敌人）
- 🔦 **准星绿光/红光**：枪管准星与屏幕准星默认发**绿光**，玩家进入狂暴模式时自动切换为**红光**（枪口发光点 + HUD 准星同步变色）
- 🖐️ **手型重做**：持枪/换弹的手部模型从简单方拳改为**手掌 + 4 指（抓握弯曲指节）+ 拇指**，更接近真实人手形状；左右手、换弹左手操作动画保留
- 🧪 测试钩子：pigSpeedTest 扩展 defense=2、新增 crosshairTest / pigBossPriorityTest；test_v83.py 全量通过（含回归）

### v8.2（2026-09-06）
- 🐗 **猪头佳强化**：生命值再提升 2 倍（12000 → **24000**）；超时自爆爆炸范围 ×2（半径 10→**20**）、爆炸伤害 ×2（130→**260**）、对玩家范围伤害 ×2（5+15·(1-d/14)→**10+30·(1-d/28)**，范围 14→28）
- 🔦 **猪头佳激光俯角动态化**：向地面发射的激光束俯角从固定 30°~50° 扩大为 **35°~75°** 动态往复扫射（落点在地面的扫射范围更广）
- 🙌 **双手持枪（CS 风格）**：所有武器新增右手（握把）+ 左手（护木/枪身）双持画面；换弹时左手自动移向弹匣/护木操作（抽装弹匣、泵动护木），换弹结束复位——手枪/步枪/霰弹/喷火器/狙击/火箭筒全部支持
- 🧪 测试钩子：enemyHealthTest（猪 24000）、pigLaserPitchSweepTest（35°~75°）、新增 handsTest（6 枪双持）；gunClipTest 跳过手部 mesh（手臂从屏幕边伸入是正常 CS 视角表现）；test_v82.py 全量通过（含回归）

### v8.1（2026-09-06）
- 📻 **空中支援上限 1 → 3**：对讲机可重复拾取累计（拾第 1 个记 1 次、再捡对讲机或每 5000 分奖励额外 +1，最多 3 次）；空中支援满 3 次后不再掉落/拾取，使用后 -1；新增 `airCount()` 统一计数
- 🧱 **穿模优化**：`collideBigEntities` 从仅处理猪头佳/BOSS 扩展到**所有敌人**（普通敌人/蜘蛛/猪/BOSS），彻底消除玩家走进敌人体内的穿模；并加 5 米距离预过滤，避免每帧对远处敌人做 AABB 计算
- ⚡ **性能优化（继续）**：火焰变色复用全局 `THREE.Color`（消除每帧临时对象）；爆炸火球/内焰/冲击环/烟雾共享几何（烟雾改用 scale 控制尺寸）；火花冲击环几何缓存
- 🧪 测试钩子：awardMilestoneTest/itemCapTest 更新为上限 3 语义；test_v81.py 全量通过（含回归）

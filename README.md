...
一个**零依赖、纯前端、程序化生成**的第一人称射击游戏（当前版本 **v11.5**）。双击 `index.html` 即可游玩 —— 无服务器、无构建、无安装。过关模式（5 关）+ 无尽模式，墙体掩体结构、全场景可破坏、猪头佳/机甲 BOSS 双 BOSS 战。
...
- 🐗 **猪头佳小BOSS**：每击杀 10 名普通敌人随机出现（几率 −30%），**55000 血量**、移动速度 2.7，双眼**绿色激光** + 无差别冲撞，**优先攻击玩家**；被障碍卡住时会**撞碎障碍物**继续追杀
